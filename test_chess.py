...
class TestPawnMovement:
    def test_white_pawn_single_forward(self):
        b = ChessBoard()
        assert b.move_piece(6, 0, 5, 0)  # a2 -> a3
        assert b.get_piece(5, 0) == 'P'
        assert b.get_piece(6, 0) == ' '

    def test_white_pawn_double_forward(self):
        b = ChessBoard()
        assert b.move_piece(6, 0, 4, 0)  # a2 -> a4
        assert b.get_piece(4, 0) == 'P'

    def test_white_pawn_double_forward_blocked(self):
        b = ChessBoard()
        b.set_piece(5, 0, 'p')  # Block the path
        assert not b.move_piece(6, 0, 4, 0)

    def test_pawn_cannot_move_backward(self):
        b = make_empty_board()
        b.set_piece(3, 0, 'P')
        b.turn = 'white'
        assert not b.move_piece(3, 0, 4, 0)

    def test_black_pawn_single_forward(self):
        b = ChessBoard()
        b.turn = 'black'
        assert b.move_piece(1, 0, 2, 0)  # a7 -> a6
        assert b.get_piece(2, 0) == 'p'

    def test_black_pawn_double_forward(self):
        b = ChessBoard()
        b.turn = 'black'
        assert b.move_piece(1, 0, 3, 0)
    ...
    def test_pawn_cannot_capture_forward(self):
        ...
        b.set_piece(3, 3, 'p')
        b.turn = 'white'
        assert not b.move_piece(4, 3, 3, 3)
    ...
    def test_pawn_diagonal_capture(self):
        ...
        b.set_piece(3, 4, 'p')
        b.turn = 'white'
        assert b.move_piece(4, 3, 3, 4)
        assert b.get_piece(3, 4) == 'P'
        assert b.get_piece(4, 3) == ' '
    ...
    def test_pawn_cannot_capture_own_piece(self):
        ...
        b.set_piece(3, 4, 'P')
        b.turn = 'white'
        assert not b.move_piece(4, 3, 3, 4)
...
class TestKnightMovement:
    ...
    def test_knight_can_jump_over_pieces(self):
        ...
        assert b.move_piece(7, 1, 5, 0)  # Knight jumps over pawns
...
class TestTurnEnforcement:
    def test_white_cannot_move_black_piece(self):
        b = ChessBoard()
        assert not b.move_piece(1, 0, 2, 0)  # Try to move black pawn on white's turn

    def test_black_cannot_move_white_piece(self):
        b = ChessBoard()
        b.turn = 'black'
        assert not b.move_piece(6, 0, 5, 0)
...
class TestEnPassant:
    ...
    def test_white_en_passant_capture(self):
        ...
        b.set_piece(7, 4, 'K')
        b.set_piece(0, 4, 'k')
        # White pawn on 5th rank (row 3), black pawn about to double-push
        b.set_piece(3, 4, 'P')
        b.set_piece(1, 3, 'p')
        b.turn = 'black'
        b.move_piece(1, 3, 3, 3)  # d7 -> d5 (double push)
        assert b.en_passant_target == (2, 3)

        # White captures en passant
        assert b.move_piece(3, 4, 2, 3)
        assert b.get_piece(2, 3) == 'P'  # White pawn moved
        assert b.get_piece(3, 3) == ' '  # Black pawn captured
        assert b.get_piece(3, 4) == ' '  # Original square cleared

    def test_black_en_passant_capture(self):
        b = make_empty_board()
        b.set_piece(7, 4, 'K')
        b.set_piece(0, 4, 'k')
        b.set_piece(4, 3, 'p')  # Black pawn on 4th rank (row 4)
        b.set_piece(6, 4, 'P')  # White pawn on 2nd rank (row 6)
        b.turn = 'white'
        b.move_piece(6, 4, 4, 4)  # e2 -> e4 (double push)
        assert b.en_passant_target == (5, 4)

        # Black captures en passant
        assert b.move_piece(4, 3, 5, 4)
        assert b.get_piece(5, 4) == 'p'
        assert b.get_piece(4, 4) == ' '  # White pawn captured
    ...
    def test_en_passant_only_available_immediately(self):
        ...
        b.set_piece(3, 4, 'P')
        b.set_piece(1, 3, 'p')
        b.set_piece(6, 0, 'P')  # Extra pawn for tempo
        b.set_piece(1, 7, 'p')  # Extra pawn for tempo
        b.turn = 'black'
        b.move_piece(1, 3, 3, 3)  # d7 -> d5

        # White plays something else
        b.move_piece(6, 0, 5, 0)  # a2 -> a3
        # Black plays something
        b.move_piece(1, 7, 2, 7)  # h7 -> h6

        # Now white tries en passant — too late, should fail
        assert not b.move_piece(3, 4, 2, 3)
    ...
    def test_en_passant_score_tracking(self):
        ...
        b.move_piece(3, 4, 2, 3)  # En passant capture
    ...
    def test_en_passant_undo(self):
        ...
        b.turn = 'black'
        b.move_piece(1, 3, 3, 3)
        b.move_piece(3, 4, 2, 3)  # En passant

        b.undo_move()
        assert b.get_piece(3, 4) == 'P'  # White pawn back
        assert b.get_piece(3, 3) == 'p'  # Black pawn restored
        assert b.get_piece(2, 3) == ' '  # Destination cleared
...
class TestCastling:
    ...
    def test_castle_blocked_by_king_moved(self):
        ...
        assert not b.move_piece(7, 4, 7, 6)
    ...
    def test_castle_undo_kingside(self):
        ...
        b.undo_move()
        assert b.get_piece(7, 4) == 'K'
        assert b.get_piece(7, 7) == 'R'
        assert b.get_piece(7, 6) == ' '
        assert b.get_piece(7, 5) == ' '
    ...
    def test_castle_undo_queenside(self):
        ...
        b.undo_move()
        assert b.get_piece(7, 4) == 'K'
        assert b.get_piece(7, 0) == 'R'
        assert b.get_piece(7, 2) == ' '
        assert b.get_piece(7, 3) == ' '
        assert not b.king_moved['white']
...
class TestPawnPromotion:
    ...
    def test_promotion_with_capture(self):
        ...
        b.set_piece(0, 1, 'r')
        b.turn = 'white'
        assert b.move_piece(1, 0, 0, 1)
        assert b.get_piece(0, 1) == 'Q'
...
class TestCheck:
    ...
    def test_game_status_check(self):
        ...
        b.set_piece(1, 3, 'r')
        b.turn = 'black'
        b.move_piece(1, 3, 7, 3)  # Rook to d1, checking white king
...
class TestCheckmate:
    def test_scholars_mate(self):
        ...
        b.set_piece(7, 0, 'q')  # Queen on a1
        b.set_piece(6, 7, 'r')  # Rook on h2 — king has no escape
        b.turn = 'white'
    ...
    def test_back_rank_mate(self):
        b = make_empty_board()
        b.set_piece(7, 6, 'K')  # King on g1
        b.set_piece(6, 5, 'P')  # Pawns blocking escape
        b.set_piece(6, 6, 'P')
        b.set_piece(6, 7, 'P')
        b.set_piece(0, 4, 'k')
        b.set_piece(7, 0, 'r')  # Black rook delivers back rank mate
        b.turn = 'white'
        b.update_game_status()
        assert b.game_status == 'checkmate'
...
class TestStalemate:
    def test_king_stalemated(self):
        ...
        b.set_piece(7, 0, 'K')
        b.set_piece(0, 7, 'k')
        b.set_piece(6, 2, 'q')  # Blocks a2, b2
        b.set_piece(5, 1, 'r')  # Blocks b-file and row 3
        b.turn = 'white'
        b.update_game_status()
...
class TestCastlingRights:
    def test_king_move_loses_castling_rights(self):
        ...
        b.set_piece(0, 4, 'k')
        b.turn = 'white'
        b.move_piece(7, 4, 7, 3)  # King moves
    ...
    def test_rook_move_loses_castling_rights(self):
        ...
        b.set_piece(0, 4, 'k')
        b.turn = 'white'
        b.move_piece(7, 7, 7, 6)  # Kingside rook moves
...
class TestPinnedPieces:
    def test_pinned_piece_cannot_move(self):
        ...
        b4 = make_empty_board()
        b4.set_piece(7, 4, 'K')  # King e1
        b4.set_piece(4, 4, 'R')  # White rook on e4 (same file as king)
        b4.set_piece(0, 4, 'k')
        b4.set_piece(1, 4, 'r')  # Black rook on e7 attacks along e-file
        b4.turn = 'white'
        # Rook is pinned to king along e-file, cannot move off e-file
        assert not b4.move_piece(4, 4, 4, 5)  # Rook tries to move sideways
        # But can move along the pin line
        assert b4.move_piece(4, 4, 2, 4)  # Rook moves forward on e-file
